...
export default function StudentInstructionsPage() {
  ...
  useEffect(() => { (async () => { try { const { data: { session } } = await supabase.auth.getSession(); if (!session) throw new Error('Veuillez vous reconnecter.'); const response = await fetch(`${apiUrl}/api/instruction-threads`, { headers: { Authorization: `Bearer ${session.access_token}` } }); const payload = await response.json(); if (!response.ok) throw new Error(payload.detail); setItems(payload.items); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Impossible de charger les instructions.'); } finally { setLoading(false); } })(); }, []);
  return <div className="space-y-6"><PageHeader title="Instructions" subtitle="Vos échanges privés avec le professeur." />{loading ? <p className="py-12 text-center text-sm text-muted-foreground">Chargement des instructions…</p> : error ? <p className="py-12 text-center text-sm text-destructive">{error}</p> : !items.length ? <EmptyState icon={MessageSquare} title="Aucune instruction" description="Les instructions privées du professeur apparaîtront ici." /> : <div className="space-y-3">{items.map((item) => <Link key={item.id} href={`/eleve/instructions/${item.id}`}><Card className="transition-colors hover:border-primary/40"><CardContent className="flex items-center justify-between gap-4 p-4"><div><p className="font-medium text-foreground">Instruction du professeur</p><p className="mt-1 text-sm text-muted-foreground">{item.exercises ? `Exercice : ${item.exercises.title}` : 'Sans exercice associé'}</p></div><span className={item.status === 'open' ? 'text-sm font-medium text-success' : 'text-sm text-muted-foreground'}>{item.status === 'open' ? 'Ouverte' : 'Clôturée'}</span></CardContent></Card></Link>)}</div>}</div>;
}
