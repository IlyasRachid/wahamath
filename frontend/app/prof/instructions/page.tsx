...
export default function TeacherInstructionsPage() {
  ...
  useEffect(() => { void (async () => { const { data: { session } } = await supabase.auth.getSession(); if (!session) return; const response = await fetch(`${apiUrl}/api/notifications/instructions/read`, { method: 'POST', headers: { Authorization: `Bearer ${session.access_token}` } }); if (response.ok) window.dispatchEvent(new Event('wahamath-notifications-updated')); })(); }, []);
  return <div className="space-y-6"><PageHeader title="Instructions privées" subtitle="Suivez les échanges individuels avec vos élèves." />{loading ? <p className="py-12 text-center text-sm text-muted-foreground">Chargement des instructions…</p> : error ? <p className="py-12 text-center text-sm text-destructive">{error}</p> : !items.length ? <EmptyState icon={MessageSquare} title="Aucune instruction" description="Envoyez une instruction depuis la fiche d’un élève." /> : <div className="space-y-3">{items.map((item) => <Link key={item.id} href={`/prof/instructions/${item.id}`}><Card className="transition-colors hover:border-primary/40"><CardContent className="flex items-center justify-between gap-4 p-4"><div><p className="font-medium text-foreground">Instruction privée</p><p className="mt-1 text-sm text-muted-foreground">{item.exercises ? <>Exercice : {item.exercises.title}{item.exercises.chapters && <> <span aria-hidden="true">·</span> Chapitre : {item.exercises.chapters.title}</>}</> : 'Sans exercice associé'}</p></div><span className={item.status === 'open' ? 'text-sm font-medium text-success' : 'text-sm text-muted-foreground'}>{item.status === 'open' ? 'Ouverte' : 'Clôturée'}</span></CardContent></Card></Link>)}</div>}</div>;
}
