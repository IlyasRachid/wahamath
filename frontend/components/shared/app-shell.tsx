...
import { clearApiCache, invalidateCacheTags, preloadAuthenticatedData } from '@/lib/api-cache';
  ...
  useEffect(() => {
    const refreshCounts = async () => {
      ...
      if (role === 'student') {
        ...
        if (notifications.ok) {
          const nextCount = (await notifications.json()).items.filter((item: { read_at: string | null }) => !item.read_at).length;
          if (previousNotificationCount.current !== null && previousNotificationCount.current !== nextCount) invalidateCacheTags('notifications');
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(item.href === '/prof/moderation' ? reportCount : item.href.endsWith('/questions') ? questionCount : item.badge) ? (
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-md sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Link href={role === 'student' ? '/eleve/notifications' : '/prof'}>
            ...
            <Link href={role === 'student' ? '/eleve' : '/prof'}>
              <div
...
export const teacherNav: NavGroup[] = [
  {
    items: [
      ...
      { label: 'Classes', href: '/prof/classes', icon: Users },
