...
import { BookOpen, Eye, Loader2, Mail, Phone, Send, ShieldCheck, ShieldOff, Trash2, Users } from 'lucide-react';
...
import { invalidateCacheTags } from '@/lib/api-cache';
...
export function TeacherStudentRoster({ classes, onStudentDeleted, onStudentChanged }: { classes: ClassRoster[]; onStudentDeleted: (studentId: string) => void; onStudentChanged: () => Promise<void> }) {
  ...
  const [instruction, setInstruction] = useState(''); const [instructionExercise, setInstructionExercise] = useState('none'); const [sendingInstruction, setSendingInstruction] = useState(false); const [exercises, setExercises] = useState<{ id: string; title: string }[]>([]);
  ...
  const openStudent = async (student: Student) => {
    ...
    try {
      ...
      setDetails(payload); setInstruction(''); setInstructionExercise('none');
      ...
      if (exerciseResponse.ok) {
        ...
        const exerciseItems = (await exerciseResponse.json()).items as { id: string; title: string; classes?: { code?: string } | null }[];
        setExercises(exerciseItems.filter((exercise) => studentClassCodes.has(exercise.classes?.code ?? '')).map(({ id, title }) => ({ id, title })));
      }
    } catch (error) {
  ...
  const sendInstruction = async () => { if (!selectedStudent || !instruction.trim()) return; setSendingInstruction(true); try { const { data: { session } } = await supabase.auth.getSession(); if (!session) throw new Error('Veuillez vous reconnecter.'); const response = await fetch(`${apiUrl}/api/admin/students/${selectedStudent.id}/instruction`, { method: 'POST', headers: { Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ body: instruction, exercise_id: instructionExercise === 'none' ? null : instructionExercise }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.detail); invalidateCacheTags('instructions'); setInstruction(''); setInstructionExercise('none'); toast.success('Instruction envoyée à l’élève.'); } catch (error) { toast.error(error instanceof Error ? error.message : 'Impossible d’envoyer l’instruction.'); } finally { setSendingInstruction(false); } };
  ...
  return <>
    ...
    <Dialog open={Boolean(selectedStudent)} onOpenChange={(open) => { if (!open && !deleting) { setSelectedStudent(null); setDetails(null); setConfirmingDelete(false); } }}>
      <DialogContent>
        ...
        {loadingDetails ? <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : details && <div className="space-y-4"><div className="rounded-lg bg-secondary/40 p-4"><p className="text-lg font-semibold text-foreground">{details.display_name}</p><p className="mt-1 text-sm text-muted-foreground">Compte créé le {new Date(details.created_at).toLocaleDateString('fr-FR')} · {details.status === 'active' ? 'Actif' : 'Suspendu'}</p></div><dl className="space-y-3 text-sm"><div className="flex gap-3"><Mail className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><div><dt className="text-muted-foreground">E-mail</dt><dd className="font-medium text-foreground">{details.email ? <a href={`mailto:${details.email}`} className="text-primary hover:underline">{details.email}</a> : 'Non renseigné'}</dd></div></div><div className="flex gap-3"><Phone className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><div><dt className="text-muted-foreground">Téléphone</dt><dd className="font-medium text-foreground">{details.phone_number ? <a href={whatsappUrl(details.phone_number)} target="_blank" rel="noreferrer" className="text-primary hover:underline">{details.phone_number}</a> : 'Non renseigné'}</dd></div></div></dl><div className="rounded-lg border border-border p-3"><p className="text-sm font-medium text-foreground">Changer de classe</p><Select defaultValue={details.classes[0]?.id} onValueChange={(value) => void manageStudent('move_class', value)} disabled={managing}><SelectTrigger className="mt-2"><SelectValue /></SelectTrigger><SelectContent>{classes.map((item) => <SelectItem key={item.id} value={item.id}>{item.code} — {item.name}</SelectItem>)}</SelectContent></Select></div><div className="rounded-lg border border-border p-3"><p className="text-sm font-medium text-foreground">Envoyer une instruction</p><Textarea className="mt-2" value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Instruction privée pour cet élève…" /><Select value={instructionExercise} onValueChange={setInstructionExercise}><SelectTrigger className="mt-2"><SelectValue placeholder="Exercice (facultatif)" /></SelectTrigger><SelectContent viewportClassName="max-h-44"><SelectItem value="none">Aucun exercice</SelectItem>{exercises.map((exercise) => <SelectItem key={exercise.id} value={exercise.id}>{exercise.title}</SelectItem>)}</SelectContent></Select><Button className="mt-2" size="sm" onClick={() => void sendInstruction()} disabled={sendingInstruction || !instruction.trim()}><Send className="h-4 w-4" />Envoyer</Button></div>{confirmingDelete && <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3"><p className="text-sm font-medium text-destructive">Supprimer définitivement ce compte et ses données ?</p><div className="mt-3 flex justify-end gap-2"><Button size="sm" variant="outline" onClick={() => setConfirmingDelete(false)}>Annuler</Button><Button size="sm" variant="destructive" disabled={deleting} onClick={() => void removeStudent()}>Supprimer définitivement</Button></div></div>}</div>}
